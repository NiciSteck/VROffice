...
"""
!!!
THIS SCRIPT IS ONLY A COPY OF sense_making.py WITH Odometry REPLACED BY PoseWithCovarianceStamped TO HAVE A COORDINATE SYSTEM WITHOUT DRIFT (check https://www.ros.org/reps/rep-0105.html odom vs map).
!!!
However the sensemaking logic has not been adapted to this change so it might result in undesired effects because of the lower frequency of the messages sent by PoseWithCovarianceStamped.
Feel free to test and improve it.

The script is not used but only included for completeness.
"""
