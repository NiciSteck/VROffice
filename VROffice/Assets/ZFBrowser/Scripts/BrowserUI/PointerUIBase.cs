...
namespace ZenFulcrum.EmbeddedBrowser {
    ...
    {
		public static MouseInputBase INSTANCE;

		public abstract Vector2 position { get; }
		public abstract bool leftDown { get; }
		public abstract bool leftPressed { get; }
		public abstract bool leftUp { get; }
		public abstract bool rightDown { get; }
		public abstract bool rightPressed { get; }
		public abstract bool rightUp { get; }
		public abstract bool middleDown { get; }
		public abstract bool middlePressed { get; }
		public abstract bool middleUp { get; }
		public abstract Vector2 scrollDelta { get; }
	}

	public interface IExternalFeeder
	{
		void FeedPointers(PointerUIBase target);
	}
...
#region Pointer Core
	...
	protected int p_currentDown, p_anyDown, p_currentOver, p_anyOver;
		...
		//bool scrollingPrev = false;
		//Vector2 scrollDelta = Vector2.zero;
		//Vector2 position2DPrev = Vector2.zero;

	bool p_scrolling = false;
	bool p_scrollingPrev = false;
	Vector2 p_scrollPos = Vector2.zero;
	Vector2 p_scrollPosPrev = Vector2.zero;
	Vector2 p_scrollDelta = Vector2.zero;
	...
	public virtual void FeedPointerState(PointerState state) {
		...
		{
			p_scrolling = state.scrolling;
			if (p_scrolling)
			{
				p_scrollPos = state.position2D;
				if (!p_scrollingPrev)
				{
					p_scrollPosPrev = p_scrollPos;
				}

				state.activeButtons = (MouseButton)0;
				p_scrollDelta = p_scrollPos - p_scrollPosPrev;
				state.scrollDelta = 10 * new Vector2(p_scrollDelta.x, -p_scrollDelta.y);

				p_scrollPosPrev = p_scrollPos;
			}

			p_scrollingPrev = p_scrolling;
		}
...
#region Input Handlers
	...
	public IExternalFeeder externalFeeder = null;

	public delegate void ExternalInput(PointerUIBase target);
	...
	protected virtual void OnHandlePointers() {
		...
		if (externalFeeder != null)
			externalFeeder.FeedPointers(this);
	...
	protected virtual void FeedMousePointer() {
			var mouse = MouseInputBase.INSTANCE;
			if (mouse == null)
				return;
		var buttons = (MouseButton)0;
		if (mouse.leftPressed) buttons |= MouseButton.Left;
		if (mouse.rightPressed) buttons |= MouseButton.Right;
		if (mouse.middlePressed) buttons |= MouseButton.Middle;

		FeedPointerState(new PointerState {
			id = 1,
			is2D = true,
			position2D = mouse.position,
